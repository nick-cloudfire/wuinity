...
namespace PREACTcli
{
    ...
    /// </summary>
    internal static class ElmfireRunner
    {
                                 ...
                                 bool resume, TextWriter log, string gdalBinDir = null)
        {
            ...
            if (exit != 0)
            {
                result.Message = $"elmfire exited {exit}" + (stderrTail.Length > 0 ? ": " + stderrTail : "");
                return result;
            }

            if (!TryCollectOutputs(outputsDir, result))
            {
                result.Message = "elmfire reported success but produced no time_of_arrival raster in " + outputsDir;
                return result;
            }
